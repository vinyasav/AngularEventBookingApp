...
@Component({
    ...
    <br /><br />      
    <img height="50%" width="100%" [src]="book.image">
      <br />
        <h1>{{book.title}}</h1>
        <div [innerHtml]="book.description"></div>
        <button class="mat-raised-button mat-primary" color="primary" (click)="addBookToCart(book)" md-raised-button color="primary">Book Now <i class="material-icons">add_shopping_cart</i></button>
...
export class SelectedBookComponent implements OnInit {
    ...
    constructor(private store: Store<fromBook.CatalogState>, private route: ActivatedRoute,
        private router: Router, private shared:BooksService) {
    ...
    addBookToCart(book: Book) {
        this.shared.availableSeats = book.seats;
